  ...
  for (auto n : frame_tmp_) {
    ...
    if ((n.angle < 20.0) && (last_angle > 340.0)) {
      ...
      if ((count * GetSpeed()) > (kPointFrequence * 1.4)) {
        ...
        std::cout << "je sais pas mais rapide" << std::endl;
        return false;
      }
      */
      ...
      if (tmp.size() > 0) {
        //std::cout << "packet assembled" << std::endl;
        ToLaserscan(tmp);
...
void LiPkg::ToLaserscan(std::vector<PointData> src) {
  ...
  angle_max = ANGLE_TO_RADIAN(src.back().angle)-ANGLE_TO_RADIAN(speed_ / kPointFrequence);
  range_min = 0.02;
  range_max = 12;
